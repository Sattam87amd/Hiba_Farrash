...
const AchieveTheLook = () => {
  ...
  const headingText = "الهوية هي سر الجمال... ولمسة الأناقة."
;

  const images = [
    { src: "/HomeImg/hiba1.webp", alt: "عائشة عزيز" },
    { src: "/HomeImg/hiba3.webp", alt: "علياء عبادي" },
    { src: "/HomeImg/hiba4.webp", alt: "عائشة عزيز" },
    { src: "/HomeImg/hiba2.webp", alt: "عائشة عزيز" },
    { src: "/HomeImg/hiba5.webp", alt: "عائشة عزيز" },
    { src: "/HomeImg/hiba6.webp", alt: "عائشة عزيز" },
  ];
  ...
  const DesktopScrollLayout = () => (
    <div className="relative">
      {/* Gradient overlays */}
      <div className="absolute top-0 right-0 w-8 sm:w-12 md:w-16 h-full bg-gradient-to-r from-transparent to-[#EDECE8] z-10 pointer-events-none"></div>
      <div className="absolute top-0 left-0 w-8 sm:w-12 md:w-16 h-full bg-gradient-to-r from-[#EDECE8] to-transparent z-10 pointer-events-none"></div>

      {/* Scroll arrows */}
      <button
        onClick={scrollLeft}
        aria-label="انتقال يسار"
      ...
      <div ref={scrollContainerRef} className="overflow-x-auto scrollbar-hide pb-2">
        <div className="inline-flex min-w-full gap-2 sm:gap-3 md:gap-4 lg:gap-6 xl:gap-8 px-2 sm:px-3 md:px-4 flex-row-reverse">
          {images.map((image, index) => (
            <div
              key={index}
  ...
  return (
    <main className="bg-[#EDECE8] relative min-h-screen flex flex-col justify-center overflow-hidden">
      <div className="w-full mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-6 sm:py-8 md:py-10">
        <h1 className="text-center text-black uppercase text-lg sm:text-xl md:text-2xl lg:text-3xl xl:text-4xl font-semibold mb-6 sm:mb-8 md:mb-10 lg:pb-16 leading-tight font-arabic">
          {headingText}
        </h1>
