...
function GiftCard() {
  ...
  useEffect(() => {
    ...
    if (storedToken) {
      ...
      toast.error("Authentication required. Please log in as an expert.");
    }
  }, [router]);
  ...
  const handlePurchaseAttempt = async () => {
    ...
    try {
      ...
      if (response.data.paymentUrl) {
        ...
        // Redirect to payment, and after payment, ensure redirect is to expert panel status page
        const paymentUrl = new URL(response.data.paymentUrl);
        paymentUrl.searchParams.set('redirect', '/expertpanel/giftcard-purchase-status');
